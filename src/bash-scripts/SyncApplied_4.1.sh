...
#
DATABASE=aicadevel31
USER=adempiere
DBHOST=10.1.173.103
JENKINSURL=http://ci.idempiere.org/job/iDempiere4.1Daily
...
do
    ...
    psql -d $DATABASE -U $USER -h $DBHOST -q -t -c "select name from ad_migrationscript" | sed -e 's:^ ::' | grep -v '^$' | sort > /tmp/lisDB.txt
...
do
    ...
    psql -d $DATABASE -U $USER -h $DBHOST -f "$SCRIPT" 2>&1 | tee "$OUTFILE"
...
then
    ...
    do
        OUTFILE=/tmp/`basename "$i" .sql`.out
        psql -d $DATABASE -U $USER -h $DBHOST -f "$i" 2>&1 | tee "$OUTFILE"
        if fgrep ERROR: "$OUTFILE" > /dev/null 2>&1
        then
            MSGERROR="$MSGERROR\n**** ERROR ON FILE $OUTFILE - Please verify ****"
        fi
